...
class TimeSyncMessage:
    ...
    @staticmethod
    def calculate_offset(response: Dict, t4: float) -> float:
        """Calculate clock offset from sync response."""
...
class TimeSyncClient:
    ...
    def get_synchronized_time(self) -> float:
        """Get synchronized time using average offset."""
        return time.time() + self.get_average_offset()
    
    def start_background_sync(self, peers: List[Tuple[str, int, str]]):
